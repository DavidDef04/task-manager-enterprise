...
class TaskControllerIntegrationTest {
    ...
    void createTask_returns401_whenTokenIsInvalid() throws Exception {
        TaskRequest request = TaskRequest.builder().title("Task").description("desc").status(TaskStatus.TODO).dueDate(null).build();
    ...
    void taskLifecycle_createListUpdateDelete_succeeds() throws Exception {
        ...
        TaskRequest createRequest = TaskRequest.builder().title("Write tests").description("Cover edge cases").status(TaskStatus.TODO).dueDate(null).build();
        ...
        TaskRequest updateRequest = TaskRequest.builder().title("Write tests").description("Cover edge cases").status(TaskStatus.DONE).dueDate(null).build();
    ...
    void updateTask_returns404_whenTaskIdDoesNotExist() throws Exception {
        ...
        TaskRequest request = TaskRequest.builder().title("Ghost task").description("desc").status(TaskStatus.TODO).dueDate(null).build();
    ...
    void deleteTask_returns404_whenTaskBelongsToAnotherUser() throws Exception {
        ...
        TaskRequest createRequest = TaskRequest.builder().title("Private task").description("Owner only").status(TaskStatus.TODO).dueDate(null).build();
        String createBody = mockMvc.perform(post("/api/tasks")
    ...
    void createTask_returns400_whenTitleIsBlank() throws Exception {
        ...
        TaskRequest request = TaskRequest.builder().title("").description("desc").status(TaskStatus.TODO).dueDate(null).build();
